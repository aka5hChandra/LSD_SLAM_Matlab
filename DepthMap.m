classdef DepthMap < handle
    ...
    methods 
        function obj = DepthMap(K,w, h)
            ...
            obj.otherDepthMap = DepthMapPixelHypothesis(zeros(w,h) , zeros(w,h), intialVar, intialVar,validityCounter,isValid);
            obj.validityIntegralBuffer = zeros(w,h);
        ...
        function observeDepthUpdate(obj,x , y , referenceFrame)
            ...
            ids = false(obj.width, obj.height);
		...
		%increase var by a little (prediction-uncertainty);
         ...
         diff = (result_idepth - obj.currentDepthMap.idepth_smoothed(3:obj.width-3 , 3 : obj.height - 3));
                  ...
                  % if inconsistent
                   ids = false(obj.width, obj.height);    
	ids(3:obj.width-3 , 3 : obj.height - 3)  = referenceFrame.validIDs .*((globalParams.DIFF_FAC_OBSERVE.*diff.*diff) > result_var + obj.currentDepthMap.idepth_smoothed(3:obj.width-3 , 3 : obj.height - 3));
         ...
         ids2 = ids & obj.currentDepthMap.idepth_var > globalParams.MAX_VAR;
          ...
          temp  = zeros(obj.width, obj.height);
          temp(3:obj.width-3 , 3 : obj.height - 3)  = (new_idepth);
          
             ids = false(obj.width, obj.height);
           ids(3:obj.width-3 , 3 : obj.height - 3) =   referenceFrame.validIDs ;
       ...
       temp =  zeros(obj.width, obj.height);
       temp(3:obj.width-3 , 3 : obj.height - 3) = id_var;
       id_var = temp;
          id_var(3:obj.width-3 , 3 : obj.height - 3)  = (referenceFrame.validIDs .*(id_var(3:obj.width-3 , 3 : obj.height - 3) .* w));
        ...
        obj.currentDepthMap.validity_counter(3:obj.width-3 , 3 : obj.height - 3)  =  obj.currentDepthMap.validity_counter(3:obj.width-3 , 3 : obj.height - 3)  + validityInc;
             ...
             for y= yMin:yMax      	
                for x= regularize_radius:obj.width-regularize_radius
                    ...
                    %destRead = obj.otherDepthMap.isvalid(x,y)   
                     ...
                     if(~obj.otherDepthMap.isValid(x,y))
                         continue
                     end
...
%source =   obj.otherDepthMap.isValid(x +dx,y + dy) 
                        if(~obj.otherDepthMap.isValid(x +dx,y + dy)) 
                            continue;
                        end
					%stats->num_reg_total++;

					diff =obj.otherDepthMap.idepth(x +dx,y + dy) - obj.otherDepthMap.idepth(x,y);
                        if(globalParams.DIFF_FAC_SMOOTHING * diff * diff > obj.otherDepthMap.idepth_var(x +dx,y + dy) + obj.otherDepthMap.idepth_var(x ,y ) )
                            if(removeOcclusions)
						
                                if(obj.otherDepthMap.idepth(x +dx,y + dy)  >  obj.otherDepthMap.idepth(x,y))
					...
					val_sum = val_sum + obj.otherDepthMap.validity_counter(x +dx,y + dy);

                        if(removeOcclusions)
                            numNotOccluding = numNotOccluding + 1;
                        end
					 distFac = double((dx*dx+dy*dy)*regDistVar);
					 ivar = 1.0/( obj.otherDepthMap.idepth_var(x +dx,y + dy)   + distFac);

					sum = sum + (obj.otherDepthMap.idepth(x +dx,y + dy) * ivar);
                    ...
                    if(val_sum < validityTH)
                        ...
                        obj.currentDepthMap.isValid(x,y) = false;
                        %%if(enablePrintDebugInfo) stats->num_reg_deleted_secondary++;
                        obj.currentDepthMap.blackListed(x,y) = obj.currentDepthMap.blackListed(x,y) - 1;
			...
			%%update!
			obj.currentDepthMap.idepth_smoothed(x,y)= sum;
			obj.currentDepthMap.idepth_var_smoothed(x,y)= 1.0/sumIvar;
         ...
         function regularizeDepthMapFillHolesRow(obj,yMin, yMax)
            ...
            %if(keyFrameMaxGradBuf[idx]<MIN_ABS_GRAD_DECREASE) continue;
                val = obj.validityIntegralBuffer(x+2,y+2) - obj.validityIntegralBuffer(x-3,y+2) - obj.validityIntegralBuffer(x+2,y-3)- obj.validityIntegralBuffer(x-3,y-3);
                 ...
                 if((obj.otherDepthMap.blackListed(x,y )  && val > globalParams.VAL_SUM_MIN_FOR_CREATE) || val > globalParams.VAL_SUM_MIN_FOR_UNBLACKLIST)
                 ...
                 for dx = -2 :2
                     for dy = -2 : 2
                         ...
                         sumIdepthObs =  sumIdepthObs + (obj.otherDepthMap.idepth(x +dx,y + dy) /obj.otherDepthMap.idepth_var(x +dx,y + dy));
						sumIVarObs = sumIVarObs + (1.0/obj.otherDepthMap.idepth_var(x +dx,y + dy));
						num = num+1;
                     end
                 end
			 ...
			 idepthObs = sumIdepthObs / sumIVarObs;
				...
				obj.currentDepthMap.idepth(x,y) =idepthObs;
                obj.currentDepthMap.idepth_smoothed(x,y) = idepthObs;
                obj.currentDepthMap.idepth_var(x,y) =globalParams.VAR_RANDOM_INIT_INITIAL;
                obj.currentDepthMap.idepth_var_smoothed(x,y) = globalParams.VAR_RANDOM_INIT_INITIAL;
                 obj.currentDepthMap.validity_counter(x,y)  = 0;
             ...
             for y=yMin:yMax
                 ...
                 for x=0:obj.width
                   if  obj.currentDepthMap.isValid(x,y)
                     ...
                     obj.validityIntegralBuffer(x,y) = validityIntegralBufferSUM;
         ...
         function buildRegIntegralBuffer(obj)
            ...
            for  y= 2: obj.height
                for x = 1 : obj.width
                obj.validityIntegralBuffer(x, y) = obj.validityIntegralBuffer(x, y) + obj.validityIntegralBuffer(x, y-1);   
                end
            end
